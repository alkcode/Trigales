...
$sentencia = $base_de_datos->prepare("SELECT id_producto,pastel,p_temporada FROM pasteles WHERE pastel= ? LIMIT 1");
...
if($producto){
	...
	if($indice === FALSE){
		...
		if($relleno==0){
			$producto->p_temporada = $producto->p_temporada;
			
		}else{
			$producto->p_temporada = $producto->p_temporada+$rellenoValor->precio;
			$producto->pastel =	$producto->pastel." RELLENO DE ".$rellenoValor->nombreRelleno;
				...
				switch ($barquillos) {
						case 'Sin barquillos':
							...
							break;
							case '13':
								$producto->p_temporada= $producto->p_temporada+5;
								$producto->pastel=	$producto->pastel." CON 13 BARQ.";
								break;
							case '14':
								$producto->p_temporada= $producto->p_temporada+10;
								$producto->pastel=	$producto->pastel." CON 14 BARQ.";
								break;
							case 'Cubierta extra':
									$producto->p_temporada= $producto->p_temporada+8;
		...
		$producto->total = $producto->p_temporada * $cantidad;
		array_push($_SESSION["carrito"], $producto);
	}else{
		//stock
		$_SESSION["carrito"][$indice]->cantidad++;
		$_SESSION["carrito"][$indice]->total = $_SESSION["carrito"][$indice]->$cantidad * $_SESSION["carrito"][$indice]->p_temporada;
	}
