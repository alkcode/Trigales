...
$sentencia = $base_de_datos->prepare("SELECT id_producto,pastel,p_medio FROM pasteles WHERE pastel= ? LIMIT 1");
...
if($producto){
	...
	if($indice === FALSE){
		...
		//agregar por si natcasesort
			...
			if($relleno==0){
				$producto->p_medio= $producto->p_medio;

			}else{
				$producto->p_medio = $producto->p_medio+$rellenoValor->precio;
				...
				switch ($barquillos) {
						case 'Sin barquillos':
							...
							break;
							case '13':
								$producto->p_medio= $producto->p_medio+5;
								$producto->pastel=	$producto->pastel." CON 13 BARQ.";
								break;
							case '14':
								$producto->p_medio= $producto->p_medio+10;
								$producto->pastel=	$producto->pastel." CON 14 BARQ.";
								break;
							case 'Cubierta extra':
									$producto->p_medio= $producto->p_medio+8;
		...
		$producto->total = $producto->p_medio * $cantidad;
		array_push($_SESSION["carrito"], $producto);
	}else{
		//stock
		$_SESSION["carrito"][$indice]->cantidad++;
		$_SESSION["carrito"][$indice]->total = $_SESSION["carrito"][$indice]->$cantidad * $_SESSION["carrito"][$indice]->p_medio;
	}
